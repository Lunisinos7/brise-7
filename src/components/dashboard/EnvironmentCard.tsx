...
const EnvironmentCard = ({
  ...
  return (
    <Card className={cn("transition-all duration-300 hover:shadow-elevated h-full flex flex-col", !isActive && "opacity-60", isActive && activeCount > 0 ? "border-cooling/30 bg-gradient-to-br from-cooling-light/5 to-transparent" : "")}>
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {linkedEquipments.length} {t("environmentCard.equipments")}
            </p>
          </div>
          <div className="flex items-center gap-1">
            <Badge variant={!isActive ? "outline" : activeCount > 0 ? "default" : "secondary"} className="shrink-0">
              {!isActive ? `⏸️ ${t("environmentCard.paused")}` : `${activeCount}/${linkedEquipments.length} ${t("environmentCard.actives")}`}
            </Badge>
      ...
      <CardContent className="p-4 pt-2 flex flex-col flex-1">
        <div className="flex-1">
          ...
          <div className="gap-6 mb-3 flex items-start justify-start">
            <div>
              <p className="text-xs text-muted-foreground">{t("environmentCard.avgTemp")}</p>
              <p className="text-xl font-bold">{avgTemp > 0 ? `${avgTemp}°C` : "--"}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">{t("environmentCard.consumption")}</p>
              <p className="text-xl font-bold text-cooling">{(totalConsumption / 1000).toFixed(1)} kW</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">{t("environmentCard.mode")}</p>
              <p className="text-muted-foreground/60 flex items-center gap-1 font-normal text-xl">
                {environment.isAutomatic ? <>
                    <Zap className="h-4 w-4" />
                    {t("environmentCard.auto")}
                  </> : <>
                    <Settings2 className="h-4 w-4" />
                    {t("environmentCard.manual")}
                  </>}
              </p>
            </div>
          </div>
          ...
          {linkedEquipments.length > 0 && <div className="pt-2 border-t">
              <p className="text-xs text-muted-foreground mb-2">{t("environmentCard.linkedEquipments")}</p>
