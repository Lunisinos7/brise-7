...
const EditTimeRoutineDialog = ({ routine, children }: EditTimeRoutineDialogProps) => {
  const { environments } = useEnvironments();
  const { currentWorkspaceId } = useWorkspaceContext();
  const { updateRoutine } = useTimeRoutines(currentWorkspaceId || undefined);
  ...
  // Load routine data when dialog opens
  useEffect(() => {
    if (open && routine) {
      setRoutineName(routine.name);
      setSelectedEnvironments(routine.environment_ids);
      
      // Convert schedules to DaySchedule format
      const schedulesMap: Record<string, TimeSlot[]> = {};
      routine.schedules.forEach((schedule, index) => {
        if (!schedulesMap[schedule.day_of_week]) {
          schedulesMap[schedule.day_of_week] = [];
        }
        schedulesMap[schedule.day_of_week].push({
          id: `${index}`,
      ...
      const daySchedulesData: DaySchedule[] = Object.entries(schedulesMap).map(([dayId, timeSlots]) => ({
        dayId,
        timeSlots,
      }));
  ...
  const toggleDaySchedule = (dayId: string) => {
    setDaySchedules(prev => {
      ...
      if (existingDay) {
        ...
        return [...prev, {
          dayId,
          timeSlots: [{ id: Date.now().toString(), startTime: "09:00", endTime: "17:00" }]
        }];
      }
    });
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Pencil className="h-5 w-5 text-cooling" />
            Editar Rotina
          </DialogTitle>
          <DialogDescription>
            Modifique as configurações da rotina
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button 
            ...
            disabled={!routineName || daySchedules.length === 0 || selectedEnvironments.length === 0 || updateRoutine.isPending}
            onClick={handleUpdateRoutine}
          >
            {updateRoutine.isPending ? "Salvando..." : "Salvar Alterações"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
...
export default EditTimeRoutineDialog;
